...
if not plot_diff:
    ...
    # Log-log plot
    axs[0].loglog(t_values, C * (t_values ** alpha * (1 + t_values) ** (1 - alpha)) ** (r - 2) - k_t, '--', color='black', linewidth=2)
    axs[0].set_title('$C  (t^{\\alpha}  (1 + t)^{1 - \\alpha})^{r - 2} - k(t)$')
    axs[0].grid(True)

    # Standard plot
    axs[1].plot(t_values, C * (t_values ** alpha * (1 + t_values) ** (1 - alpha)) ** (r - 2) - k_t, '--', color='black', linewidth=2)
